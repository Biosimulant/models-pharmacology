...
"""Tellurium-backed SBML BioModule for Zake2021 - PBPK model of metformin in mice: single dose intavenous."""
from __future__ import annotations

from biosim.contrib.sbml import TelluriumSBMLBioModule


class Zake2021PbpkModelOfMetforminInMiceSingleBiomd0000001039Model(TelluriumSBMLBioModule):
    """Faithfully executes the bundled SBML source for Zake2021 - PBPK model of metformin in mice: single dose intavenous."""

    _SBML_ID = 'BIOMD0000001039'
    _TITLE = 'Zake2021 - PBPK model of metformin in mice: single dose intavenous'
    ...
    _PARAMETER_INPUTS = {'metformin_plasma_dose_mg': ('Metformin_Dose_in_Plasma_in_mg', 1.375, 'mg', 'IV/plasma metformin dose. Maps to SBML symbol `Metformin_Dose_in_Plasma_in_mg`.'), 'metformin_lumen_dose_mg': ('Metformin_Dose_in_Lumen_in_mg', 0.0, 'mg', 'lumen dose parameter, normally zero for IV scenario. Maps to SBML symbol `Metformin_Dose_in_Lumen_in_mg`.'), 'body_weight_g': ('ModelValue_1', 27.5, 'native SBML value', 'mouse body weight. Maps to SBML symbol `ModelValue_1`.'), 'cardiac_output_ml_per_min': ('ModelValue_3', 838.8, 'mg/min', 'cardiac output. Maps to SBML symbol `ModelValue_3`.')}
    ...
    def __init__(self, model_path: str = "data/BIOMD0000001039.xml", integration_step: float = 0.1) -> None:
